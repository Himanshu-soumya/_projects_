...
def update(frame):
    ax.cla()
    ax.scatter(range(1, frame + 1), soil_moisture_history[:frame], plant_growth_history[:frame],
               c=irrigation_history[:frame], cmap='viridis', s=50)
    ax.set_xlabel('Days')
...
# Create animation
animation = FuncAnimation(fig, update, frames=days, interval=200, repeat=False)
